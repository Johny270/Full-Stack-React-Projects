...
// find all the courses that have an instructor field that matches
// the user specified with the userId param.
...
// find the corresponding course document and update its 
// lessons array field by pushing the new lesson object received
// in the request body.
...
const update = (req, res) => {
    ...
    form.parse(req, async (err, fields, files) => {
        if (err) {
            return res.status(400).json({
                error: "Photo could no be uploaded"
            })
        }
        let course = req.course
        course = extend(courses, fields)
        if (fields.lessons) {
